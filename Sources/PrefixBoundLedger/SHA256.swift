...
/// A small, dependency-free SHA-256 so the ledger hashes identically on
/// Linux CI and on device. CryptoKit would do on Apple platforms; this keeps
/// `swift test` honest on a Linux box with no Security framework.
enum SHA256 {
    ...
    static func hexDigest(_ string: String) -> String {
        hash(Array(string.utf8)).map { String(format: "%02x", $0) }.joined()
    }
